...
public class MagicMaze {
	...
	public static void main(String[] args) throws Exception {
		...
		MagicMaze m4 = new MagicMaze(maze4, 15, 10);
    ...
    public boolean validSpace(int x, int y){
        if(x>=0 && x<rows && y>=0 && y<cols && mazee[x][y] != '@' ){
			if(Character.isDigit(mazee[x][y])&&transport.get(mazee[x][y]).get(0)>3){
				return false;
			}else if(!seen[x][y]){
				return false;
	...
	public int[] transportation(char num, int row, int col){
		int[] ans = new int[2];
		System.out.println("before trans "+ row+col);
		int numOfTrans = transport.get(num).get(0);
		transport.get(num).set(num, 1+numOfTrans);
		System.out.println("we transported to "+ row+col);
		if(transport.get(num).get(1)==row && transport.get(num).get(2)==col){
			ans[0] = transport.get(num).get(3);
			ans[1] = transport.get(num).get(4);
		} else{
			ans[0] = transport.get(num).get(1);
			ans[1] = transport.get(num).get(2);
		}
		return ans;
	}
